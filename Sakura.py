...
try:
    ...
    class Compile:
        ...
        def DitMeMayDungCoDec() -> str:
            global __AnhOiDungCoDecObject__, __selfObject__, __key__
            obj = __selfObject__
        ...
        def RemoveLayers() -> str:
            global __selfObject__, __module, ___module
    ...
    """
        ...
        """
            ...
            if len(node.targets) == 1 and isinstance(node.targets[0], ast.Subscript):
                ...
                if isinstance(subscript.value, ast.Call) and isinstance(subscript.value.func, ast.Name) and (subscript.value.func.id in ('globals', 'vars', 'locals')):
                    if isinstance(subscript.slice, ast.Constant) and isinstance(subscript.slice.value, str):
                        ...
                        if namespace_func == 'globals' and self.current_function and (var_name not in self.global_vars):
                            ...
                            assign_stmt = ast.Assign(targets=[ast.Name(id=var_name, ctx=ast.Store())], value=node.value)
                            ...
                            ast.copy_location(assign_stmt, node)
                            self.modified = True
                            comment = ast.Expr(value=ast.Constant(value=f"# Converted from {namespace_func}()['{var_name}']", kind=None))
        ...
        """
            if isinstance(node, ast.BinOp):
                ...
                node.keywords = [ast.keyword(arg=kw.arg, value=self.visit_globals_in_expr(kw.value)) for kw in node.keywords]
                return node
            elif isinstance(node, ast.Subscript) and isinstance(node.value, ast.Call):
                if isinstance(node.value.func, ast.Name) and node.value.func.id in ('globals', 'vars', 'locals') and isinstance(node.slice, ast.Constant) and isinstance(node.slice.value, str):
